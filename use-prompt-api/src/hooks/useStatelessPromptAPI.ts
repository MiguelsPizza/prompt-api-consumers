...
class PromptAPIError<T extends 'SESSION_UNAVAILABLE' | 'SESSION_CREATION_FAILED' | 'PROMPT_FAILED' | 'EMPTY_PROMPT'> extends Error implements IPromptAPIError {
...
export type UseStatelessPromptAPIError = PromptAPIError<'SESSION_UNAVAILABLE' | 'SESSION_CREATION_FAILED' | 'PROMPT_FAILED' | 'EMPTY_PROMPT'>;
...
export function useStatelessPromptAPI({ initialPrompts = [], monitor, signal, systemPrompt, temperature, topK }: AILanguageModelCreateOptionsWithSystemPrompt): StatelessPromptAPIResult {
  ...
  const sendPrompt = useCallback(async (
    ...
    try {
      if (!session.current) throw new PromptAPIError('Session not available', 'SESSION_UNAVAILABLE');
      if (streaming) {
